...
namespace Interface
{
  ...
  i_t extract (genv_t, list_t<decl_t>);
  ...
  struct I *merge_list (list_t<i_t, `H> li, list_t<stringptr_t<`H, `H>, `H> linfo);
  // same but ask for the interfaces one by one
  struct I *get_and_merge_list (i_t (@get) (`a), list_t<`a, `H> la, list_t<stringptr_t<`H, `H>, `H> linfo);
